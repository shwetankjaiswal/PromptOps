...
public sealed class AppserverTools(AppserverService appserverService, AngleService angleService)
{
    ...
    public async Task<string> ExecuteTask(
        [Description("ID(integer) of the task to execute")] string task_id,
    ...
    [McpServerTool, Description("Get current status and details of a task based on its integer id")]
    public async Task<string> GetTaskStatus(
        [Description("ID(integer) of the task to check")] string task_id)
    {
        if (_appserverService == null)
            return JsonSerializer.Serialize(new { error = "AppserverService not initialized" });
    ...
    [McpServerTool, Description("Get list of all export tasks from the Appserver")]
